...
bool IsProcessedState(deque<BucketState>& states, const BucketState& newState)
{
   ...
   // it = find_if( states.begin(), states.end(),
    ...
    it = find_if(states.begin(), states.end(), bind2nd(mem_fun_ref(&BucketState::IsSameState), newState));
...
int main(int argc, char *argv[])
{
    ...
    SearchState(states);
    qDebug()<<n<<endl;
    Q_ASSERT(states.size() == 1); /*穷举结束后states应该还是只有一个初始状态*/
    return a.exec();
}
